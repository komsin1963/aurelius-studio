...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      // 🎖️ เช็ค Badge อัตโนมัติเมื่อเข้าหน้าโปรไฟล์
      ...
      // ดึงข้อมูลโปรไฟล์และเหรียญรางวัลมาโชว์
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/5 border border-white/10 rounded-[3.5rem] p-12 mb-10 relative overflow-hidden shadow-2xl">
          <div className="flex flex-col md:flex-row items-center gap-10">
            <div className="w-32 h-32 rounded-full bg-gradient-to-br from-cyan-500 to-fuchsia-600 p-1">
            ...
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center gap-3 justify-center md:justify-start mb-2">
                <span className="px-2 py-0.5 bg-cyan-500 text-black text-[8px] font-black uppercase rounded">Active Citizen</span>
                <h2 className="text-4xl font-black italic uppercase tracking-tighter">
                  #{user?.id.slice(0, 5).toUpperCase()}
                </h2>
              </div>
              <p className="text-slate-500 text-[10px] font-black tracking-widest mb-6 italic uppercase underline decoration-cyan-500/30">
                {user?.email}
              </p>
              <div className="bg-white/5 inline-flex px-6 py-3 rounded-2xl border border-white/10 font-black text-xs uppercase tracking-widest items-center gap-3">
        ...
        <div className="bg-[#0b0b12] border border-white/5 rounded-[3rem] p-10 mb-10">
          <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500 flex items-center gap-3 italic mb-10">
            <Award className="text-cyan-500" size={16} /> Neural Achievement Record
          </h3>

          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {userBadges.map((ub: any) => (
              <div key={ub.badge_id} className="group bg-white/5 border border-white/10 p-6 rounded-[2.5rem] flex flex-col items-center text-center hover:border-cyan-500 transition-all">
                <Star size={24} className="text-cyan-500 mb-4" />
                <p className="text-[10px] font-black uppercase text-white mb-1">{ub.badges?.title}</p>
        ...
        <button 
          ...
          className="w-full py-8 rounded-[2.5rem] bg-red-500/5 border border-red-500/10 text-red-500 font-black uppercase tracking-[0.5em] text-[12px] hover:bg-red-600 hover:text-white transition-all duration-500 flex items-center justify-center gap-4"
        >
